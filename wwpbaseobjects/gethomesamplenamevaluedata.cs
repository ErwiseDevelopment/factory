...
namespace GeneXus.Programs.wwpbaseobjects {
   public class gethomesamplenamevaluedata : GXWebProcedure
   {
      public override void webExecute( )
      {
         ...
         initialize();
         Gxm3rootcol = new GXBaseCollection<GeneXus.Programs.wwpbaseobjects.SdtHomeSampleNameValueData_HomeSampleNameValueDataItem>( context, "HomeSampleNameValueDataItem", "Factory2") ;
         ...
         if ( StringUtil.StrCmp(StringUtil.Lower( GXSoapHTTPRequest.Method), "get") == 0 )
         {
            if ( StringUtil.StrCmp(StringUtil.Lower( GXSoapHTTPRequest.QueryString), "wsdl") == 0 )
            {
               ...
               GXSoapXMLWriter.WriteAttribute("targetNamespace", "Factory2");
               GXSoapXMLWriter.WriteAttribute("xmlns:wsdlns", "Factory2");
               ...
               GXSoapXMLWriter.WriteAttribute("xmlns:tns", "Factory2");
               ...
               GXSoapXMLWriter.WriteAttribute("soapAction", "Factory2action/"+"wwpbaseobjects.AGETHOMESAMPLENAMEVALUEDATA.Execute");
            ...
            else
            {
               ...
               currSoapErrmsg = "No SOAP request found. Call " + ((context.GetHttpSecure( )==1) ? "https://" : "http://") + context.GetServerName( ) + ((context.GetServerPort( )>0)&&(context.GetServerPort( )!=80)&&(context.GetServerPort( )!=443) ? ":"+StringUtil.LTrim( StringUtil.Str( (decimal)(context.GetServerPort( )), 6, 0)) : "") + context.GetScriptPath( ) + "wwpbaseobjects.gethomesamplenamevaluedata" + "?wsdl to get the WSDL.";
            }
         }
         if ( currSoapErr == 0 )
         {
         ...
         if ( ( StringUtil.StringSearch( currMethod+"&", "Execute&", 1) > 0 ) || ( currSoapErr != 0 ) )
         {
            ...
            GXSoapXMLWriter.WriteAttribute("xmlns", "Factory2");
            if ( currSoapErr == 0 )
            {
               if ( Gxm3rootcol != null )
               {
                  Gxm3rootcol.writexmlcollection(GXSoapXMLWriter, "ReturnValue", "Factory2", "HomeSampleNameValueDataItem", "Factory2");
               }
            }
            else
            {
      ...
      {
         this.Gxm3rootcol = new GXBaseCollection<GeneXus.Programs.wwpbaseobjects.SdtHomeSampleNameValueData_HomeSampleNameValueDataItem>( context, "HomeSampleNameValueDataItem", "Factory2") ;
         initialize();
         ExecuteImpl();
