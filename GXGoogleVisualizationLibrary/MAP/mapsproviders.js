function GoogleProvider(mapControl) {
    ...
    if (mapControl.MarkerClustering){
		 var renderer = {
			render({ count, position }, stats) {
				var url;

				if (count <= 10){
					url = gx.util.resourceUrl(gx.basePath + gx.staticDirectory + "GXGoogleVisualizationLibrary/MAP/Images/Smallclusterer.svg", true);
				}else if (10 < count && count < 100 ){
					url = gx.util.resourceUrl(gx.basePath + gx.staticDirectory + "GXGoogleVisualizationLibrary/MAP/Images/Mediumclusterer.svg", true);
				}else{
					url = gx.util.resourceUrl(gx.basePath + gx.staticDirectory + "GXGoogleVisualizationLibrary/MAP/Images/Largeclusterer.svg", true);
				}

				var label
				if (mapControl.DisplayNumberClusteredPoints){
					if (mapControl.DisplayStyleClusteredPoints){
						label = {
								className: mapControl.DisplayStyleClusteredPoints,
								text: String(count),
								color:"" , 
								fontSize:"" ,
								fontFamily: "",
        ...
        marker_Clusterer.push(new markerClusterer.MarkerClusterer({ map, markers, renderer}));
		}		
		// Lines
		var gMapLines = mapControl.GoogleMap.Lines;
		if (gMapLines && gMapLines.length > 0) {
			for (var i = 0; gMapLines[i] != undefined; i++) {
				...
				for (var j = 0; gMapLines[i].Points[j] != undefined; j++) {
					polygonLinePath[j] = new google.maps.LatLng(gMapLines[i].Points[j].PointLat, gMapLines[i].Points[j].PointLong);
                                ...
                                var LineInfowin = gMapLines[i].LineInfowinHtml;
				var polyline = new google.maps.Polyline({
					path: polygonLinePath, 
                                        htmlinfo: LineInfowin,
	...
	function clearAllMarkers() {
		...
		Circles = [];
		for (var i = 0; i < marker_Clusterer.length; i++) {
			marker_Clusterer[i].setMap(null);
		}		
		marker_Clusterer = [];
	}
