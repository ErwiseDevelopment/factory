            ...
            if ( StringUtil.StrCmp(AV10SDMapearTags.gxTpr_Tabela, "Empresa") == 0 )
            {
               ...
               while ( (pr_default.getStatus(5) != 101) )
               {
                  ...
                  while ( AV29GXV4 <= AV10SDMapearTags.gxTpr_Tags.Count )
                  {
                     ...
                     new debug(context ).execute(  StringUtil.Format( "&SdMapearTagsTags %1", AV11SdMapearTagsTags.gxTpr_Tag, "", "", "", "", "", "", "", "")) ;
    ...
    private void cursorDefinitions( )
    {
       if ( def == null )
       {
          ...
          def= new CursorDef[] {
             ...
             ,new CursorDef("P009713", "SELECT T1.EmpresaBancoId AS EmpresaBancoId, T1.EmpresaNomeFantasia, T1.EmpresaRazaoSocial, T2.BancoNome, T1.EmpresaAgencia, T1.EmpresaAgenciaDigito, T1.EmpresaConta, T1.EmpresaPix, T1.EmpresaPixTipo, T1.EmpresaEmail, T1.EmpresaId FROM (Empresa T1 LEFT JOIN Banco T2 ON T2.BancoId = T1.EmpresaBancoId) ORDER BY T1.EmpresaId ",false, GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK, false, this,prmP009713,100, GxCacheFrequency.OFF ,true,false )
             ,new CursorDef("P009714", "SELECT ContratoId, ContratoClienteId, ContratoTaxa, ContratoSLA, ContratoJurosMora, ContratoIOFMinimo FROM Contrato WHERE ContratoId = :AV14ContratoId ORDER BY ContratoId ",false, GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK, false, this,prmP009714,1, GxCacheFrequency.OFF ,true,true )
