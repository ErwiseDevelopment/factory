...
namespace GeneXus.Programs {
   public class prhtmlfimcontrato : GXProcedure
   {
      public prhtmlfimcontrato( )
      {
      ...
      public prhtmlfimcontrato( IGxContext context )
      {
      ...
      protected override void ExecutePrivate( )
      {
         ...
         AV8HTML += ".signature-card { background-color: #fff; border: 1px solid #ddd; border-radius: 5px; padding: 15px; margin-bottom: 15px; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1); }";
         ...
         AV8HTML += "        </div>";
         AV17GXV1 = 1;
         while ( AV17GXV1 <= AV10Array_SdParticipantesContrato.Count )
         {
            AV9SdParticipantesContrato = ((SdtSdParticipantesContrato)AV10Array_SdParticipantesContrato.Item(AV17GXV1));
            AV8HTML += "    <div class=\"container\">";
            AV8HTML += "        <h3>";
            if ( String.IsNullOrEmpty(StringUtil.RTrim( AV9SdParticipantesContrato.gxTpr_Participanterepresentantenome)) )
            {
               GXt_char1 = AV16Nome;
               new initcap(context ).execute(  StringUtil.Trim( AV9SdParticipantesContrato.gxTpr_Participantenome), out  GXt_char1) ;
               AV16Nome = GXt_char1;
            }
            else
            {
               GXt_char1 = AV16Nome;
               new initcap(context ).execute(  StringUtil.Trim( AV9SdParticipantesContrato.gxTpr_Participantenome), out  GXt_char1) ;
               AV16Nome = StringUtil.Format( " %1 representada por %2", GXt_char1, AV9SdParticipantesContrato.gxTpr_Participanterepresentantenome, "", "", "", "", "", "", "");
            }
            AV8HTML += StringUtil.Format( "            <span class=\"check-icon\"><i class=\"fas fa-check\"></i></span> %1", AV16Nome, "", "", "", "", "", "", "", "");
            AV8HTML += "        </h3>";
