...
gx.evt_i = (function($) {
	...
	checkFuncKey: function (evt, el) {
		var currentPopup = gx.popup.getPopup();
		var gxO = currentPopup ? currentPopup.frameWindow.gx.O : gx.O;
		var fKey = this.lastKey - 111;
		if (evt.shiftKey) {
			fKey += 12;
		}
		if (fKey > 0 && fKey <= 24 && (this.keyListeners[fKey] != undefined)) {
			var gxEvent = this.keyListeners[fKey][gxO.CmpContext];
			if (typeof (gxEvent) != 'undefined') {
				this.execFnKeyEvt(evt, gxO, gxEvent, el);
			}
			else {
	...
	setCurrentGridRows: function( GridId, rowId) {
		while (GridId > 0 && rowId && rowId.length >= 4) {
			gx.fn.setCurrentGridRow(GridId, rowId.substr( 0, 4));
			var parentRow = rowId.substr( 4),
				gridObj = gx.O.getGridById(GridId, parentRow);
			if (gridObj) {
...
gx.http_i = (function ($) {
		...
		return {
			...
			isSessionTimeoutError: function (req, exc) {
				if (!gx.gen.isDotNet() && !gx.gen.isDotNetCore()  && (exc && req) && gx.pO.AjaxSecurity) {
			...
			doCall: function (info) {
				...
				if (info.multipart && info.method == 'POST') {
					...
					if (req != null) {
						...
						req.setRequestHeader( gx.util.browser.GX_TAB_ID_HEADER, gx.util.browser.tabId);
						if (info.method == 'POST')
			...
			formatLink: function (url, parms, pparamNames) {
				var paramNames = gx.csv.useNamedParameters === undefined || gx.csv.useNamedParameters === true ? pparamNames : undefined;
				...
				if (parms && parms.length > 0) {
					...
					for (var i = 0; i < len; i++) {
						...
						var data = gx.http.encodeUriSegment(typeof(parms[i].getUrlVal) == 'function' ? parms[i].getUrlVal() : parms[i]);
						data = ((i === len - 1) && (data === "") && (gx.csv.useNamedParameters === false)) ? ',' : data;
						url += parmName + data;						
					}
				}
				return url;
...
var GlobalEvents = (function ($) {
	return {		
		executeMethod: function(methodName, parms) {
			try {
				var doNotify = function(w) {
					if (w.gx) {
						w.gx.fx.obs.notify(methodName, parms);
					}
				}
				doNotify(window.top);
				const iframes = window.top.document.querySelectorAll('iframe');
				iframes.forEach((iframe, index) => {
					doNotify(iframe.contentWindow);
				});
			}
			catch (e) {
				gx.dbg.logEx(e, 'gxapi.js', 'GlobalEvents.executeMethod');
			}
		}
	};
})(jQuery);
...
(function($) {
		...
		var anyError = function (vStruct, pGrid, pRow) {
			...
			if (vStruct) {
				if (grid && row && vStruct.grid == grid) {
					...
					element = vStruct.fld ? gx.fn.getControlGridRef(vStruct.fld, vStruct.grid) : null;
	...
	prot.executeServerEvent = function (EvtName, Synch, EvtRow, Force, GlobalContextEvent) {
		...
		gx.evt.execEvt(this.CmpContext, this.IsMasterPage, evtName, gx.evt.dummyCtrl, gridId, synchReq, null, true, afterFnc, Force, failFnc, false);
		return deferred.promise();
	};
	...
	prot.executeEnterEvent = function (evt, ctrl, disableForm) {
		...
		if (gxOEnter != null) {
			...
			var callback = function() {
				gx.evt.executingEnterEvent = false;
			}
			gx.evt.executingEnterEvent = true;
			gx.evt.execEvt(gxOEnter.CmpContext, gxOEnter.IsMasterPage, enterName, gx.evt.dummyCtrl, gridId, false, null, disableForm, callback, forceEnter, callback, gxOEnter != null);
			if (evt) {
				gx.evt.cancel(evt, true);
			}
		}
	...
	prot.startFeedback = function(immediately, swallowKeys) {
		if(gx.pO.MasterPage && this === gx.pO.MasterPage){
			return this.startFeedback.apply(gx.pO,arguments)
		}
	...
	prot.endFeedback = function() {
		if(gx.pO.MasterPage && this === gx.pO.MasterPage){
			return this.endFeedback.apply(gx.pO)
		}
		gx.fn.enableKeys();
		...
		ext: {													
			...
			popUp: function (popupObj, autoSize, popCentered, x, y, w, h, cid, text, bgcolor, textcolor, fontstyleset, title, titlecolor, titletextcolor, bordercolor, scrollcolor, shadowcolor, ismodal, showonstart, isdrag, isresize, oldOK, isExt, popOnce, minImg, maxImg, clsImg, rsImg, headCls, btnCls, cntCls, brdCls, shdwCls, showParentPopups) {
				...
				if (!popupObj.IsPrompt && !gx.util.sameAppUrl(text)) {
					...
					text += encodeURIComponent('gxTabId=' + gx.util.browser.tabId + ';');
					var	contentHtml = '',
...
gx.ajax = (function ($) {
	...
	return {
			...
			return {
				...
				getInputParms: function () {
					...
					for (var i = 0, len = parmsMetadata.length; i < len; i++) {
						...
						else if (this.gxO.isTransaction() && this.grid > 0 && this.row && parm.av === 'Gx_mode' && parm.fld === 'vMODE')
						{
										...
										{ 
											if (gx.lang.isArray(value)) { //Collection based grid
												value = {       
													pRow: parentRow,               
													c: value,
													v: value[Number(gx.fn.currentGridRowImpl(parm.grid)) - 1] || value[0]
												};
											}
										}
							...
							inputParms.push(gx.lang.emptyforType(value, parm.type));
						}
					}
				...
				doAjaxCall: function (Synch, disableForm) {
					...
					if (disableForm !== false) {
						this.gxO.startFeedback( false, true);
					}
		...
		enableForm: function (gxO, bForced) {
			gxO = gxO || gx.O;
			if (bForced) {
				gxO.feedbackCallCounter = 0;
			}
			if (gx.pO)
				gx.pO.endFeedback();
			if (gxO != gx.pO)
